...
class _LoginPageState extends State<LoginPage> {
  ...
  LoginOnClick() async{
    if (username.text.length != 0 || password.text.length != 0) {
      ...
      try {
        var response = await http.post('${ServerUrl}/token/',
            headers: {
        ...
        if (response.statusCode == 200) {
          ...
          User = json_response['full_name'];
          UserUID = json_response['access'];
        } else {
          LoadingStop(context);
